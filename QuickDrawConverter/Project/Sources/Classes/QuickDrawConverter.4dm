...
Class constructor($controller : 4D:C1709.Class)
	...
	If (Not:C34(OB Instance of:C1731($controller; cs:C1710._QuickDrawConverter_Controller)))
		$controller:=cs:C1710._QuickDrawConverter_Controller
	End if 
	
	Super:C1705("QuickDrawConverter"; $controller)
...
Function convert($option : Variant; $formula : 4D:C1709.Function) : Collection
	...
	This:C1470.controller.dataType:="blob"
	...
	For each ($option; $options)
		...
		Case of 
			...
			: ((Value type:C1509($option.file)=Is object:K8:27) && (OB Instance of:C1731($option.file; 4D:C1709.Blob))) || (Value type:C1509($option.file)=Is BLOB:K8:12)
				$command+=" - "
				$isStream:=True:C214
		End case 
		
		If (Not:C34($isStream))\
			 && (Value type:C1509($option.output)=Is object:K8:27)\
			 && (OB Instance of:C1731($option.output; 4D:C1709.File))\
			 && ($option.file.path#$option.output.path)
			$command+=" "
